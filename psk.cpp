...
namespace psk
{
	...
	void send_psk(float baud, unsigned int freq, unsigned char vol, gpio::Pin* ptt_pin, const char* text) {
		...
		ptt_pin->set(true);	// enable PTT
		
		send_preamble(sine, aplay, baud);
		
		for (unsigned int i = 0; i < strlen(text); i++) {	// send the message
			send_char(text[i], sine, aplay);
		}
		
		send_postamble(sine, aplay, baud);
		
		ptt_pin->set(false);	// turn off PTT
		
		pclose(aplay);
	}

	void send_aprs(unsigned int freq, unsigned char vol, gpio::Pin* ptt_pin, const char* source, unsigned char source_ssid, const char* destination, unsigned char destination_ssid, const char* payload) {
		stringstream buff;
